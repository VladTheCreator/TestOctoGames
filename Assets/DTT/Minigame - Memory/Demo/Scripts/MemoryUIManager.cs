...
namespace DTT.MinigameMemory.Demo
{
    ...
    public class MemoryUIManager : MonoBehaviour
    {
        ...
        {
            if (!PlayerPrefs.HasKey("gameWithNona"))
            {
                PlayerPrefs.SetInt("gameWithNona", 1);
            }
            ...
            if (PlayerPrefs.GetInt("gameWithNona") == 0)
            {
                _finishedMenu._homeButton.onClick.RemoveAllListeners();
